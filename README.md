# Bouchra (aloyzero)

Web developer  focusing mostly on trying to create fun and useful websites , ai applied webapps 

![Typing SVG](https://readme-typing-svg.demolab.com/?font=Orbitron&size=20&center=true&vCenter=true&width=650&color=00F5FF&lines=Running+vision+systems+through+the+neon+haze;Teaching+machines+to+see+what+humans+miss;Building+the+future%2C+one+glitchy+commit+at+a+time)

[![GitHub followers](https://img.shields.io/github/followers/aloyzero?style=flat-square&label=Followers)](https://github.com/aloyzero?tab=followers)
[![Profile views](https://komarev.com/ghpvc/?username=aloyzero&style=flat-square&label=Profile+views)](https://github.com/aloyzero)

---

## Tech Stack

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=flat-square&logo=opencv&logoColor=white)
![Electron](https://img.shields.io/badge/Electron-47848F?style=flat-square&logo=electron&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)

---

## Featured Projects

**[haris](https://github.com/aloyzero/haris)**
AI-powered surveillance platform using YOLOv8 and MediaPipe for real-time detection, behavior analysis, alerts, and security analytics.
`Python` `YOLOv8` `MediaPipe` `Computer Vision`

**[hand-gesture-control](https://github.com/aloyzero/hand-gestures-)**
Real-time computer vision system for controlling Windows volume and mouse navigation through hand gestures.
`Python` `OpenCV` `MediaPipe`

**[docnow](https://github.com/aloyzero/docnow)**
Healthcare platform for finding doctors, comparing service costs, and scheduling appointments online.
`JavaScript` `Web App`

**[ticktactoe](https://github.com/aloyzero/ticktactoe)**
A lightweight, dependency-free Tic Tac Toe game with a dark gradient interface and full cross-device compatibility.
`HTML5` `CSS3` `Vanilla JS`

**[pizzamusicplayer](https://github.com/aloyzero/pizzamusicplayer)**
Desktop music player built with the Electron framework.
`Electron` `JavaScript`

---

## Contribution Status

![GitHub Streak](https://streak-stats.demolab.com/?user=aloyzero&theme=default&hide_border=true)

<p align="center"><i>Thanks for stopping by  may your builds always pass on the first try.</i></p>
